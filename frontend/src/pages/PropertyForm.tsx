...
export default function PropertyForm() {
  ...
  const [form, setForm] = useState({
    ...
    description: '', highlights: '', featured: false,
    tags: '', amenities: '',
  })
  ...
  useEffect(() => {
    if (isEdit) {
      ...
      propertiesAPI.get(Number(id)).then(({ data }) => {
        setForm({
          title: data.title || '', type: data.type || 'apartamento',
          purpose: data.purpose || 'venda', status: data.status || 'disponivel',
          address: data.address || '', neighborhood: data.neighborhood || '',
          city: data.city || '', state: data.state || '', zip_code: data.zip_code || '',
          area: data.area || '', built_area: data.built_area || '',
          bedrooms: data.bedrooms || '0', bathrooms: data.bathrooms || '0',
          suites: data.suites || '0', parking_spaces: data.parking_spaces || '0',
          floor: data.floor || '', price: data.price || '',
          rent_price: data.rent_price || '', condominium_fee: data.condominium_fee || '0',
          iptu: data.iptu || '0', description: data.description || '',
          highlights: data.highlights || '', featured: data.featured || false,
          tags: (data.tags || []).join(', '),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        condominium_fee: Number(form.condominium_fee),
      ...
      if (isEdit) {
        await propertiesAPI.update(Number(id), payload)
      } else {
            ...
            <div className="card">
              ...
              <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <Field label="Descrição">
                  <textarea className="form-textarea" value={form.description} onChange={e => set('description', e.target.value)} placeholder="Descreva o imóvel em detalhes..." style={{ minHeight: '120px' }} />
                </Field>
            ...
            <div className="card">
              ...
              <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <div className="form-grid">
                  ...
                  <Field label="UF">
                    <input className="form-input" value={form.state} onChange={e => set('state', e.target.value)} maxLength={2} placeholder="SP" />
                  </Field>
                  <Field label="CEP">
              ...
              <div className="card-body">
                <div className="form-grid form-grid-3" style={{ gap: '14px' }}>
                  <Field label="Área total (m²)">
                    <input type="number" className="form-input" value={form.area} onChange={e => set('area', e.target.value)} />
                  </Field>
                  <Field label="Área construída (m²)">
                    <input type="number" className="form-input" value={form.built_area} onChange={e => set('built_area', e.target.value)} />
                  </Field>
                  <Field label="Quartos">
                    <input type="number" className="form-input" value={form.bedrooms} onChange={e => set('bedrooms', e.target.value)} min="0" />
                  </Field>
                  <Field label="Banheiros">
                    <input type="number" className="form-input" value={form.bathrooms} onChange={e => set('bathrooms', e.target.value)} min="0" />
                  </Field>
                  <Field label="Suítes">
                    <input type="number" className="form-input" value={form.suites} onChange={e => set('suites', e.target.value)} min="0" />
                  </Field>
                  <Field label="Vagas">
                    <input type="number" className="form-input" value={form.parking_spaces} onChange={e => set('parking_spaces', e.target.value)} min="0" />
                  </Field>
                  <Field label="Andar">
                    <input type="number" className="form-input" value={form.floor} onChange={e => set('floor', e.target.value)} />
                  </Field>
                </div>
                <div style={{ marginTop: '14px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  <Field label="Tags (separadas por vírgula)">
                    <input className="form-input" value={form.tags} onChange={e => set('tags', e.target.value)} placeholder="piscina, churrasqueira, academia, reformado" />
                  </Field>
            ...
            <div className="card">
              <div className="card-body">
                <div className="flex items-center justify-between">
                  <div>
                    <div style={{ fontWeight: 600, fontSize: '0.9rem' }}>⭐ Imóvel em Destaque</div>
                    <div className="text-sm text-muted mt-1">Exibido em posição privilegiada</div>
                  </div>
                ...
                <div className="card-body">
                  {photos.length === 0 ? (
                    <div className="empty-state" style={{ padding: '24px' }}>
                      ...
                      <p style={{ fontSize: '0.8rem' }}>Nenhuma foto</p>
                    </div>
                  ) : (
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px' }}>
                      {photos.map((photo, idx) => (
                        <div key={idx} style={{ position: 'relative', borderRadius: 'var(--radius-sm)', overflow: 'hidden', aspectRatio: '4/3' }}>
                          <img src={`http://localhost:8000${photo}`} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                          {idx === 0 && <div style={{ position: 'absolute', top: 4, left: 4, background: 'var(--accent)', borderRadius: 3, padding: '2px 6px', fontSize: '0.65rem', fontWeight: 700, color: 'var(--bg-base)' }}>CAPA</div>}
