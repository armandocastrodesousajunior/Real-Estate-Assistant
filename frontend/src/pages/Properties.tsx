...
import { Search, Plus, Bed, Bath, Car, MapPin, Edit, Trash2 } from 'lucide-react'
...
const TYPE_LABEL: Record<string, string> = {
  ...
  terreno: 'Terreno', rural: 'Rural', kitnet_studio: 'Kitnet/Studio',
}
    ...
    <div className="page-container">
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p>Cadastre seu primeiro imóvel para começar</p>
          <button className="btn btn-primary" onClick={() => navigate('/properties/new')}>
        ...
        <>
            ...
            {properties.map((p) => (
              <div key={p.id} className="property-card">
                <div className="property-card-img">
                  {p.cover_photo ? (
                    <img src={`http://localhost:8000${p.cover_photo}`} alt={p.title} />
                  ) : (
                  ...
                  <div className="property-specs">
                    {p.bedrooms > 0 && (
                      <div className="property-spec">
                        <Bed size={13} /> {p.bedrooms} quarto{p.bedrooms !== 1 ? 's' : ''}
                      </div>
                    )}
                    {p.bathrooms > 0 && (
                      <div className="property-spec">
                    ...
                    {p.parking_spaces > 0 && (
                      <div className="property-spec">
                        <Car size={13} /> {p.parking_spaces}
                      </div>
                    )}
                    {p.area && (
                      <div className="property-spec">{p.area}m²</div>
                    )}
                  </div>
          ...
          {total > 12 && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '24px' }}>
              ...
              <span style={{ padding: '6px 14px', background: 'var(--bg-elevated)', borderRadius: 'var(--radius-md)', fontSize: '0.85rem' }}>
