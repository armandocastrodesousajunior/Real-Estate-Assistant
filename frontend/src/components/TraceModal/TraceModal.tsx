      ...
      <div className="modal-content card" onClick={e => e.stopPropagation()} style={{
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold flex items-center gap-2" style={{ color: '#FFFFFF', margin: 0 }}>
            <Search size={22} /> Rastreamento de IA
          </h2>
          <button 
        ...
        {showJson ? (
          ...
          <div className="timeline" style={{ display: 'flex', flexDirection: 'column', gap: '20px', position: 'relative' }}>
            ...
            <div className="timeline-icon" style={{
                ...
                display: 'flex', alignItems: 'center', justifySelf: 'center', color: '#000000', flexShrink: 0
              }}>
                <User size={20} />
              </div>
              <div className="timeline-content" style={{ flex: 1, background: 'rgba(255,255,255,0.03)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border)' }}>
                ...
                <p className="text-sm text-muted mt-2" style={{ lineHeight: 1.5 }}>
                  Analisou a mensagem do cliente e redirecionou para: <span className="badge badge-muted" style={{ color: '#FFF' }}>{trace.supervisor_selection}</span>.
                </p>
              </div>
            </div>
            ...
            {trace.calls && trace.calls.filter(c => !c.success).map((c, i) => (
              <div key={i} className="timeline-item" style={{ display: 'flex', gap: '16px', zIndex: 1, position: 'relative' }}>
                <div className="timeline-icon" style={{
                  ...
                  display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#FFFFFF', flexShrink: 0, border: '1px solid rgba(255,255,255,0.1)'
                }}>
                  <ArrowRight size={20} />
                </div>
                <div className="timeline-content" style={{ flex: 1, background: 'rgba(255,255,255,0.03)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border)' }}>
                  <div className="text-xs text-muted font-mono mb-1">Passo {i + 2} • Avaliação e Recusa</div>
                  <div className="font-semibold" style={{ color: '#FFFFFF' }}>Agente {c.agent_slug}</div>
                  <div className="mt-3 p-3 rounded" style={{ background: 'rgba(255,255,255,0.05)', borderLeft: '3px solid #FFF' }}>
            ...
            <div className="timeline-item" style={{ display: 'flex', gap: '16px', zIndex: 1, position: 'relative' }}>
              ...
              <div className="timeline-content" style={{ flex: 1, background: 'rgba(255,255,255,0.03)', padding: '16px', borderRadius: '8px', border: '1px solid var(--border)' }}>
                ...
                <div className="font-semibold text-white">Agente {trace.final_agent}</div>
                <p className="text-sm text-muted mt-2" style={{ lineHeight: 1.5 }}>
